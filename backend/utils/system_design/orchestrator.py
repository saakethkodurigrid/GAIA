...
Remember: You are helping them discover answers through questions, not providing answers directly."""
    ...
    async def check_proactive_prompts(self, session: Session) -> Optional[str]:
        ...
        # 2️⃣ Event-Driven (Figure-Aware): Significant canvas change + idle for ≥10s
        if session.current_canvas and session.previous_canvas:
            ...
            if is_significant:
                # Check if user has been idle for 10 seconds AFTER the significant change
                ...
                if session.last_drawing_activity_time:
                    ...
                    print(f"[DEBUG] Event-Driven: significant change detected, idle_time={idle_time:.1f}s since change (need >=10s)")
                    
                    if idle_time >= 10:  # 10 seconds idle after significant change
                        ...
                        print(f"[DEBUG] Event-Driven: idle_time too short ({idle_time:.1f}s < 10s)")
                else:
